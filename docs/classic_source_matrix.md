...
| source availability | **YES — author-published and byte-pinned**: canonical `iwbtgbeta(fs).mfa` fetched from `kayin.moe` on 2026-09-01; 85,300,282 bytes; SHA-256 `c41928c…78f76`; pointer repo `Kayinnasaki/IWBTG` @ `bb5a6cb5`; Google Drive is transport-only and must match the pin | NO — only an unofficial, **incomplete** community decompile ("missing a few frames", speedrun.com forum) | NO | **YES — author's own public repo** `SUDALV92/K2W` (re-verified: HEAD `a6d6dce1`, 157 rooms, 1469 objects, 227 scripts hands-on from a fresh clone) | NO — official free itch download only | NO (searched again 2026-08-30: nothing) | NO — official itch downloads alive, no project files | NO (download dead) | NO (original channel expired; only unofficial mirrors/mobile knockoffs) |
