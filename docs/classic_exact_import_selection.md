...
| source checksum | recorded at import time from the user-fetched file (kayin.moe is unreachable from the build sandbox; the open-directory inventory — names, sizes, dates — is pinned in the manifest now, and the importer will refuse any file whose sha256 does not then match the recorded value on first registration). Pointer repo pinned today: `Kayinnasaki/IWBTG` @ `bb5a6cb5`. |
| importer adapter required | new `mfa2pack`: an offline Clickteam-format reader on the open-source `mmfparser` lineage (Anaconda project, GPL — used as an external tool, not linked into the env) → frames, object instances, movements, event sheets → IR → the existing gamepack pipeline. Event sheets lower to the exact-layer op/trigger machinery the GM track already uses; MMF-specific behaviors become native classes. |
